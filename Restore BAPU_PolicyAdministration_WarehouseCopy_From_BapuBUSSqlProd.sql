...
/****** Object:  Job [Restore BAPU_PolicyAdministration_WarehouseCopy_From_BapuBUSSqlProd]    Script Date: 03/17/2021 4:09:02 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Database Maintenance]    Script Date: 03/17/2021 4:09:03 PM ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Step 1: Kill User Connections]    Script Date: 03/17/2021 4:09:03 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Step 1: Kill User Connections', 
	...
	ALTER DATABASE [BAPU_PolicyAdministration] SET SINGLE_USER WITH ROLLBACK IMMEDIATE', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Step 2: Drop Existing BAPU PolicyAdministration Database]    Script Date: 03/17/2021 4:09:04 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Step 2: Drop Existing BAPU PolicyAdministration Database', 
	...
	drop database BAPU_PolicyAdministration', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Step 3: Delete Existing Backup File]    Script Date: 03/17/2021 4:09:04 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Step 3: Delete Existing Backup File', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Step 4: Copy Backup File From BapuBUSSqlProd]    Script Date: 03/17/2021 4:09:04 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Step 4: Copy Backup File From BapuBUSSqlProd', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Step 5: Restore BAPU PolicyAdministration Database]    Script Date: 03/17/2021 4:09:04 PM ******/
...
move ''ftfg_Organization.DoingBusinessAs.FTCatalog_EC684D2'' to ''D:\sqldata\BAPU_Organization.DoingBusinessAs.FTCatalog'', replace', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Step 6: Set BAPU PolicyAdministration to Multi-User Access]    Script Date: 03/17/2021 4:09:04 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Step 6: Set BAPU PolicyAdministration to Multi-User Access', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Step 7: Set Recovery Model to Simple]    Script Date: 03/17/2021 4:09:04 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Step 7: Set Recovery Model to Simple', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Step 8: Set Permissions]    Script Date: 03/17/2021 4:09:04 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Step 8: Set Permissions', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'EXEC sys.sp_addrolemember ''db_datareader''
                         ,''WRBTS\Primary IT Support Team'';

EXEC sys.sp_adduser ''WRBTS\svc-bap-p-dw''
                   ,''WRBTS\svc-bap-p-dw'';

EXEC sys.sp_addrolemember ''db_datareader''
                         ,''WRBTS\svc-bap-p-dw'';

EXEC sys.sp_addrolemember ''db_datareader''
                         ,''WRBTS\BAPU IT'';

EXEC sp_adduser ''bapu_olap''
               ,''bapu_olap'';
...
EXEC sys.sp_changedbowner ''sa'';', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Step 9: Update Statistics]    Script Date: 03/17/2021 4:09:05 PM ******/
...
EXECUTE dbo.sp_updatestats', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Step 10: Run Fix User's Script]    Script Date: 03/17/2021 4:09:05 PM ******/
